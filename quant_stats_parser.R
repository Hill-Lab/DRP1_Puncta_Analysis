...
#open a file explorer window for the user to select their directory containing data files
...
subDir_list <- c("/quant_puncta_allZ/", "/quant_metrics_allZ/")#, "/quant_metrics_maxIP/")
path_list <- NULL
file_list <- NULL
path_files <- list()
raw_data <- NULL
...
#find each subdirectory and generate a single dataframe from all files within
for (i in seq(1, length(subDir_list))) {
  #look for existing workbooks and avoid redundant processing if found
  ...
  if (!file.exists(workbook_names[i])) {
    #check to make sure subdirectory is valid
    if (dir.exists(paste(root, subDir_list[i], sep = ""))) {
      path_list[i] <- (paste(root, subDir_list[i], sep = ""))
      
      #identify all .csv files within subdirectory
...
# 
# setwd(root)
# raw_data_quant_metrics_maxIP <- raw_data_list[[1]]
# raw_data_quant_puncta_maxIP <- raw_data_list[[2]]
