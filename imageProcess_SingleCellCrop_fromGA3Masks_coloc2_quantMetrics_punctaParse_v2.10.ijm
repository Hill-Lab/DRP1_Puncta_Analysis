...
criteriaMethod = newArray("ELONGATION", "COMPACTNESS", "VOLUME", "MSER", "EDGES");
thresholdMethod = newArray("STEP", "KMEANS", "VOLUME");
...
//iterate through GA3_masks and map file paths using previously set up unique identifiers
    ...
    for (j = 0; j < numROIs; j++) {
      ...
      for (k = 0; k < origChannels; k++) {          
        if (channelSelected[k]) {
          ...
          for (l = 0; l < origSlices; l++) {
            ...
            //if enabled, gather quantification analysis measurements
            //if (quantAnalysisSelected[k]) run("Measure");
          }
      ...
      //if enabled, create masks of the selected channel for downstream arithmetic
          if (mitoMasking && (k == maskChannelPos)) {                       
            ...
            // roiManager("Deselect");
            // run("Z Project...", "projection=[Max Intensity]");
            // temp_maxIP = getImageID();

            // //pre-handle maxIP logic if also selected, saving compute later
            // if (maxIPStack) {              
            ...
            //run("Dilate", "stack");
            //save(punctaParse_3D_masks + "mitoMask_" + key + "_" + IJ.pad(j+1,2) + ".tif");
            //resetThreshold;
            ...
            run("Convert to Mask", "background=Dark list create");
            //run("Erode", "stack");
            //save(punctaParse_3D_masks + "cytoMask_" + key + "_" + IJ.pad(j+1,2) + ".tif");
            //resetThreshold;
...
//could add info for thresholding parameters
//could add info for analyze particles parameters
//could add info on slice statistics 
...
function advancedPunctaParse(ImageTitle, SavePath, Key, Type, ROI_index, Beta) {
  ...
  //all of the above settings could be user-defined on macro setup if desired
...
//helper function for manual file type selection if autodetection fails
