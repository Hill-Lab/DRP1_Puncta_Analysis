...
//the root folder MUST contain a directory named "ROIs"
//the following is an example of the naming convvention for ROI folders: "B4-006.zip"
if (!File.isDirectory(rootFolder + "ROIs\\")) {
  print("No \"ROIs\" folder exists within " + rootFolder + ".\nTerminating macro execution.");
  exit;
}
roiFolder = rootFolder + "ROIs\\";

//create output folder for cropped cell images
...
roiFileList = getFileList(roiFolder);
roiFileListLength = roiFileList.length;
if (roiFileListLength < 1) {
  print("No ROIs found within " + roiFolder + ".\nTerminating macro execution.");
  exit;
}
...
for (i = 0; i < originalImagesFileListLength; i++) {  
  ...
  if (endsWith(originalImageFileName, imageFileType) && i == 0) {
    ...
    if (mitoMasking && numSelectedChannels > 1) {
      ...
      Dialog.createNonBlocking("Mitochondrial Masking Parameters");
      Dialog.setInsets(0,13,0);
      Dialog.addMessage("Select channel(s) to be used in mask arithmetic", 12);
      for (j = 0; j < origChannels; j++) {
    ...
    if (punctaParse && numSelectedChannels > 0) {
      ...
      Dialog.addCheckbox("Global parsing", false);
      Dialog.addCheckbox("Mask parsing", false);
      Dialog.show();
  ...
  //this code section ensures that a raw image file and its corresponding ROIset are indexed together and will be opened simultaneously during processing
  //the following is an example of the naming convention for raw image files: "_Wells-B4_Points-006.nd2"
  //the following is an example of the naming convention for GA3 mask files: "B4-006.zip"
  ...
  if (matches(originalImageFileName,"^.*retake_[Ww]ells-[a-zA-Z][0-9]{1,2}_[Pp]oints-[0-9]{3}\." + replace(imageFileType, ".", "") + "$")) {
    ...
    keyList = keyList + key + "~~~";
      ...
      roiFilePath = roiFolder + key + ".zip";
      roiManager("Open", roiFilePath);
      numROIs = roiManager("count");
      totalROIs = totalROIs + numROIs;
      List.set(key, List.get(key) + roiFilePath);
  }
    ...
    for (j = 0; j < numROIs; j++) {
      ...
      if (colocAnalysis && colocOffset > 0) {
        for (k = 0; k < colocOffset; k++) {          
          ...
          run("Coloc 2", "channel_1=[" + _roi_title[colocCh1] + "] channel_2=[" + _roi_title[colocCh2] + "] roi_or_mask=[ROI(s) in channel 1] threshold_regression=Bisection manders'_correlation costes'_significance_test psf=1 costes_randomisations=100");
          selectWindow("Log");
          ...
          if (maxIPStack) {
            ...
            roiManager("Select", j);  
            run("Coloc 2", "channel_1=[" + _roi_maxIP_title[colocCh1] + "] channel_2=[" + _roi_maxIP_title[colocCh2] + "] roi_or_mask=[ROI(s) in channel 1] threshold_regression=Bisection manders'_correlation costes'_significance_test psf=1 costes_randomisations=100"
            selectWindow("Log");
...
//handle any failed regex matches or mappings /////////////////////////////// NEED LOG FILE OUTPUT HERE
if (roisProcessed == 0) print(runLog, "No ROIs were processed.");
  ...
  outputText = outputText + "Finished cropping a total of " + roisProcessed + roiString + numSelectedChannels + channelString + origSlices + sliceString
