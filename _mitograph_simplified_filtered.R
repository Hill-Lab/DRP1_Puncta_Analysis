...
data_raw <- read_csv(file = "outputSummary_filtered.csv")
...
for (i in seq(3, length(PlotsToBeMade))) {
  ...
  plot(ggplot(data=data_formatted,aes_string(x=xaxis, y=yaxis, fill=fill)) + 
         ...
         geom_point(data=data_formatted,aes_string(colour = fill, x=xaxis, y=yaxis), pch=20, size=0.01, position=position_jitterdodge(jitter.width=.15, jitter.height=0, dodge.width=1)) +
  ...
  ggsave(paste(widthSummaryFolder, "/Plot-", yaxis, "_filtered.eps", sep = ""), width = 40, height = 30, units = "cm", dpi = 300)
  ggsave(paste(widthSummaryFolder, "/Plot-", yaxis, "_filtered.png", sep = ""), width = 40, height = 30, units = "cm", dpi = 300)
}

data_formatted_melt <- reshape2::melt(data = data_formatted, id.vars = "Condition", measure.vars = c("PHI",
                                                                                                     "Avg_Edge_Length_um",
                                                                                                     "Total_Node_Norm_to_Length_um",
                                                                                                     "Total_Connected_Components_Norm_to_Length_um",
                                                                                                     "Free_Ends",
...
ggplot(data_formatted_melt, aes(fill=Condition, x=variable, y=value)) +
  ...
  stat_boxplot(geom="errorbar", position = position_dodge (width = 1), width = 0.5, size = 0.25) +
  #scale_fill_manual(values = c("#FF2D00","#FFE200","#001BFF","#FF2D00", "#FFE200", "#001BFF", "#FF2D00","#FFE200", "#001BFF","#FF2D00","#FFE200", "#001BFF","#FF2D00","#FFE200", "#001BFF")) +
  facet_wrap(~variable, scales = "free")+
  geom_point (aes(colour = Condition, x=variable, y=value), pch=20, position=position_jitterdodge(jitter.width=0.15, jitter.height=0, dodge.width=1), size = 0.01) +
  #scale_colour_manual(values = c("#424242","#424242","#424242","#424242","#424242","#424242","#424242","#424242","#424242","#424242","#424242","#424242","#424242","#424242","#424242")) +
  theme_bw() +
...
ggsave(paste(widthSummaryFolder, "/All_metrics-filtered.eps", sep = ""), width = 25, height = 16, units = "cm", dpi = 300)
ggsave(paste(widthSummaryFolder, "/All_metrics-filtered.png", sep = ""), width = 25, height = 16, units = "cm", dpi = 300)

aov_df <- data_formatted %>%
  select(c("Condition",
           "Treatment",
           "PHI",
           "Avg_Edge_Length_um",
...
aov_list <- list()
for (i in seq(3, ncol(aov_df))) {
  aov <- TukeyHSD(aov(data = aov_df, aov_df[, i]~(Treatment*Condition)))
  aov_table <- (aov)
  aov_list[[substr(toString(names(aov_df)[i], width = NULL), 1, 31)]] <- as.data.frame(aov_table[3:3])
}

write.xlsx(aov_list, file = paste(widthSummaryFolder, "/AOV_stats_full.xlsx", sep = ""), rowNames = TRUE)

#aov <- TukeyHSD(aov(data = data_formatted, MitoGraph_Connectivity_Score~(Treatment*Condition)))
#aov_table <- (aov)
#aov_summary <- as.data.frame(aov_table[3:3])
#write.csv(aov_summary, "MitoGraph_CS_AOV_stats.csv")
#formattable(aov_summary, Condition.p.adj=formatter("span", style = x~style(color=ifelse(x < 0.05 , "green", "black"))))